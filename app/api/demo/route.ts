...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Demo request successfully received. Our specialist will contact you shortly.",
      lead: saved,
    });
  } catch (error) {
    console.error("Error in /api/demo:", error);
    return NextResponse.json(
      { error: "Internal server error saving demo request." },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  ...
  const demoLeads = leads.filter((l) => l.type === "demo");
